    ...
    function renderRecentTestimonials(testimonials) {
        ...
        container.innerHTML = testimonials.slice(0, 4).map(t => {
            ...
            const avatarHtml = t.user_avatar
                ? `<img src="/storage/${t.user_avatar}" alt="${t.user_name}" class="w-10 h-10 rounded-full object-cover flex-shrink-0 ring-2 ring-white shadow-sm">`
                : `<div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0 ring-2 ring-white shadow-sm">
                        <span class="text-white font-semibold text-sm">${t.user_name.charAt(0).toUpperCase()}</span>
                   </div>`;

            // Trajet départ -> destination
            const departure = t.user_country || 'Non spécifié';
            const destination = t.destination || 'Non spécifié';

            return `
            <div class="p-4 flex items-start gap-3 hover:bg-slate-50/50 transition-colors">
                ${avatarHtml}
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-1.5 mt-1">
                        <span class="text-xs text-slate-500">${getCountryFlag(departure)}</span>
                        <span class="text-[10px] text-slate-400 font-medium">${departure}</span>
                        <svg class="w-3 h-3 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                        </svg>
                        <span class="text-xs text-slate-500">${getCountryFlag(destination)}</span>
                        <span class="text-[10px] text-indigo-600 font-medium">${destination}</span>
                    </div>
                    <p class="text-xs text-slate-500 line-clamp-1 mt-1.5">${t.content}</p>
                    <div class="flex items-center gap-2 mt-2">
                        <span class="text-[10px] px-1.5 py-0.5 rounded-full ${t.is_approved ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}">${t.is_approved ? 'Approuvé' : 'En attente'}</span>
